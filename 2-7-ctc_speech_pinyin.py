...
def get_model(img_w=32, img_h=20, output_size=None, max_pred_len=4,model_path='best.h5'):
    ...
    y_pred=Conv1D(kernel_size=1,filters=output_size,padding="same",activation="softmax",kernel_regularizer=regularizers.l2(0.2))(logit)
...
def filter_blank_text(line, tok=None):
    ...
    if len(line.split(' ')) != len(tok.texts_to_sequences([line])[0]):
        return False
    return True
...
#     name_to_seq, tok = get_thchs_corpus(path2,wav_files, maxlen=48)
    ...
    name_to_pinyin2 = {}
    for k,v in name_to_text2.items():
        name_to_pinyin2[k] = ' '.join([word[0] for word in pinyin(v)])
        
    name_to_pinyin1 = {}
    for k,v in name_to_text1.items():
        name_to_pinyin1[k] = ' '.join([word[0] for word in pinyin(v)])
        
    pinyin_lines = list(name_to_pinyin2.values())[:]

    tok_pinyin = Tokenizer()
    tok_pinyin.fit_on_texts(pinyin_lines)
#     tok_pinyin.texts_to_sequences(pinyin_lines)
    
#     tok = get_token(name_to_text2, path3)
    
    print(len(name_to_pinyin1))
    name_to_pinyin1 = dict(filter(lambda x: filter_blank_text(x[1], tok=tok_pinyin), [(k, v)  for k,v in name_to_pinyin1.items()])) 
    print(len(name_to_pinyin1))
    name_to_pinyin1.update(name_to_pinyin2)
    
    name_to_seq = get_name_to_seq(name_to_pinyin1, tok_pinyin, maxlen=48)
    ...
    model, test_func = get_model(img_w=778, img_h=26, 
                                 output_size=len(tok_pinyin.word_index) + 2, max_pred_len=48,
                                 model_path=model_path)
    
    
    
    mms_batch = get_batch(train2_wavs[:10000] + train1_wavs[:10000], name_to_seq,
                          batch_size=1000, max_pred_len=48, input_length=778, 
                          mms=None)
    
    mms = get_mms(path3, mms_batch)
    
    train_batch = get_batch(train2_wavs[:10000] + train1_wavs[:10000], name_to_seq,
    ...
    train_callback = get_batch(train2_wavs[:10000] + train1_wavs[:10000], name_to_seq,
    ...
    idx2w = dict((i,w) for w,i in tok_pinyin.word_index.items())
    ...
    callback_list = [checkpointer, tfboard, csv_to_log, mcb_train, mcb_val, lr_change] 
    history = model.fit_generator(train_batch,
                                  validation_data=(val_batch),
                                  validation_steps=5,
                                  steps_per_epoch=200, epochs=1000,
