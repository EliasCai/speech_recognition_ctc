...
from utils import get_thchs_wav_paths,get_thchs_corpus
...
#    path = 'data_aishell'
    ...
    wav_files = get_thchs_wav_paths(path)
    wav_len = len(wav_files)
    train_wavs = wav_files[:int(0.8*wav_len)]
    val_wavs = wav_files[int(0.8*wav_len):int(0.9*wav_len)] 
    test_wavs = wav_files[int(0.9*wav_len):] 
    
    name_to_seq, tok = get_thchs_corpus(path,wav_files, maxlen=48)
    ...
    train_batch = get_batch(train_wavs[:], name_to_seq,
    ...
    train_callback = get_batch(train_wavs[:], name_to_seq,
                            batch_size=16, max_pred_len=48, input_length=778, 
                            mms=mms)
    
    val_callback = get_batch(val_wavs[:], name_to_seq,
                          batch_size=16, max_pred_len=48, input_length=778, 
                          mms=mms)
    ...
    csv_to_log = CSVLogger(join(path, "logger_0628.csv"),append=True)
